...
**Arquivos novos:**
- `netlify/functions/next-whatsapp.mjs` — Netlify Function no formato moderno ("Functions 2.0",
  `export default async () => {...}`, ESM). Lê/incrementa um contador no **Netlify Blobs**
  (`getStore({ name: "whatsapp-round-robin", consistency: "strong" })`, chave
...
**Pegadinha que consumiu boa parte da depuração (registrar pra não cair de novo):** a
primeira versão foi escrita no formato antigo ("Lambda compatibility", `exports.handler`).
Nesse formato o Netlify **não injeta automaticamente** o contexto do Blobs — dá o erro
`MissingBlobsEnvironmentError`. Mesmo adicionando `connectLambda(event)` (correção documentada
pro modo antigo), a leitura com `consistency: "strong"` falhava com outro erro
(`uncachedEdgeURL`) porque o contexto do modo Lambda é incompleto. A solução foi **migrar pro
formato moderno** (`.mjs`, `export default`) — nesse formato o Blobs funciona out-of-the-box,
sem `connectLambda` e com `consistency: "strong"` funcionando normalmente. **Qualquer function
nova neste site deve usar o formato moderno desde o início**, não o `exports.handler`.
...
alguns navegadores bloqueiam como pop-up. Corrigido localmente pra `window.location.href`
(navega na mesma aba), mas essa versão do arquivo ainda não foi enviada ao GitHub.

---

## 0.3 NOVIDADE DE 2026-07-28 (3ª rodada) — Google Ads (gtag.js)

Adicionado o tag do Google Ads (`AW-18142105077`) no `<head>` de **11 páginas** (todas as
páginas públicas, exceto `qr.html` que é noindex/ferramenta interna, e `1.html` que é um
arquivo órfão não referenciado por nenhum link do site):
`index.html`, `energia-por-assinatura.html`, `assessor.html`, `blog.html`,
`como-reduzir-conta-de-luz.html`, `energia-por-assinatura-vale-a-pena.html`,
`mercado-livre-de-energia-residencial-2028.html`, `quem-somos.html`, `cartao.html`,
`cartao-junior.html`, `404.html`.

Snippet colado logo após a tag `<head>`, antes de qualquer outra coisa (posição recomendada
pelo Google). **Ao criar qualquer página nova no site, adicionar esse mesmo snippet.**

**Pegadinha registrada (verificador do Google Ads não detectava a tag):** a tag estava
presente e correta no HTML o tempo todo (confirmado por fetch direto), mas o verificador do
Google Ads não conseguia detectar. Causa: o **Bot Fight Mode do Cloudflare** (ligado nesse
domínio, ver seção 2) estava bloqueando/desafiando o verificador. Resolvido desligando o Bot
Fight Mode temporariamente. **Se no futuro algum verificador externo (Google, Meta, Bing,
qualquer ferramenta de SEO/ads) disser que "não encontra" algo que sabemos que está no HTML,
suspeitar do Bot Fight Mode primeiro antes de investigar o código.**
...
Existe também um `netlify/functions/next-whatsapp.js` (extensão `.js`, versão antiga/de
debug) que ficou **órfão** localmente depois da migração pra `.mjs` — provavelmente não
existe mais no GitHub (foi "renomeado" lá), mas se aparecer duplicado checar e apagar.

---

**Também resolvido nessa rodada:** foto do cartão do Josmair (`/cartao`) reenquadrada —
a foto original (688×1488px) era muito alongada, então o círculo do avatar só mostrava 46%
da altura da imagem (rosto OU gravata, não os dois). Pré-cortada para 688×930px (do cabelo
até a gravata) e `fotoFoco` ajustado para `"50% 65%"` em `cartao.html`. **Pendência do
backlog (seção 9) sobre "foto quebrada do Josmair" está resolvida — atualizar lá também.**
...
- [x] **Foto do cartão do Josmair quebrada:** RESOLVIDO em 2026-07-28. `foto.jpg` foi
      re-subida (pré-cortada 688×930px) e `fotoFoco` ajustado para `"50% 65%"` em
      `cartao.html`. Ver seção 0.3.
